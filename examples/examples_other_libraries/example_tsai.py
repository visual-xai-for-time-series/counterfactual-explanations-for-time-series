"""
Example: Using tsai InceptionTime Classifier with cfts

This example demonstrates how to:
1. Load a time series dataset using tsai
2. Train an InceptionTime deep learning classifier
3. Integrate tsai models with cfts for counterfactual explanations

NOTE: This example uses SETS with tsai's InceptionTime classifier.
SETS is a gradient-free subsequence-based method that works well with
PyTorch-based models. Other gradient-free alternatives include MOC,
GLACIER, Multi-SpaCE, and TSEvo.

Requirements:
    pip install tsai torch matplotlib numpy

References:
- tsai: https://timeseriesai.github.io/tsai/
- InceptionTime: https://timeseriesai.github.io/tsai/models.inceptiontime.html
"""
...
# Import tsai components - only what we need to avoid unnecessary dependencies
...
class TsaiDatasetWrapper:
    """Wrapper to make tsai dataset compatible with cfts methods."""
    
    def __init__(self, X, y):
        """
        Args:
            X: Time series data (n_samples, n_channels, n_timepoints)
            y: Labels (n_samples,)
        """
...
class InceptionTimeWrapper(nn.Module):
    """Wrapper to make InceptionTime compatible with cfts methods."""
    
    def __init__(self, model, device='cpu'):
        """
        Args:
            model: Trained tsai InceptionTime model
    ...
    def forward(self, x):
        ...
        if x.ndim == 1:
            x = x.reshape(1, 1, -1)  # (1, 1, n_timepoints)
        elif x.ndim == 2:
            if x.shape[0] > 1 and x.shape[1] > x.shape[0]:
                # Likely (batch, n_timepoints), need to add channel dimension
                x = x.unsqueeze(1)  # (batch, 1, n_timepoints)
            elif x.shape[0] == 1:
                # (1, n_timepoints), need to add channel dimension
                x = x.unsqueeze(0)  # (1, 1, n_timepoints)
            else:
                # Likely (n_timepoints, n_channels) or (batch, n_timepoints)
                if x.shape[0] > x.shape[1]:
                    x = x.T.unsqueeze(0)  # (1, n_channels, n_timepoints)
                else:
                    x = x.unsqueeze(0)  # (1, n_channels, n_timepoints)
...
def load_and_prepare_data(dataset_name="FordA"):
    """
    Load and prepare time series dataset using tsai.
    ...
    """
    print(f'Loading {dataset_name} dataset from UCR archive...')
    
    # Load dataset using tsai
    X, y, splits = get_UCR_data(dataset_name, return_split=False)
    ...
    if y_train.dtype.kind in ['U', 'O', 'S']:  # Unicode, Object, or Byte string
...
def train_or_load_model(X_train, y_train, X_test, y_test, model_path='inception_time_tsai.pth'):
    """
    ...
    if os.path.exists(model_path):
        ...
        # Create dataloaders
        dls = TSDataLoaders.from_dsets(train_ds, test_ds, bs=64)
        
        # Create learner with cross-entropy loss for classification
    ...
    with torch.no_grad():
        # Training accuracy
        train_preds = model(torch.from_numpy(X_train).float())
        train_acc = accuracy_score(y_train, torch.argmax(train_preds, dim=1).numpy())
        
        # Test accuracy
        test_preds = model(torch.from_numpy(X_test).float())
        test_acc = accuracy_score(y_test, torch.argmax(test_preds, dim=1).numpy())
...
def generate_counterfactual_sets(model, dataset, sample, target_class=None):
    ...
    print('(SETS is a gradient-free method compatible with PyTorch models)')
...
def visualize_results(original, counterfactual, original_pred, cf_pred, save_path='tsai_cf_example.png'):
    """
    ...
    if isinstance(counterfactual, torch.Tensor):
        counterfactual = counterfactual.cpu().detach().numpy()
    if isinstance(original_pred, torch.Tensor):
        original_pred = original_pred.cpu().detach().numpy()
    if isinstance(cf_pred, torch.Tensor):
        cf_pred = cf_pred.cpu().detach().numpy()
...
def main():
    ...
    print('='*80)
    print('tsai InceptionTime + cfts Integration Example')
    print('='*80)
    print()
    ...
    model_path = os.path.join(script_path, 'inception_time_tsai_forda.pth')
    ...
    model_wrapper = InceptionTimeWrapper(inception_model, device=device)
    dataset_wrapper = TsaiDatasetWrapper(X_test, y_test)
    ...
    for idx in range(len(X_test)):
        sample = X_test[idx, 0, :].astype(np.float32)  # Get flattened univariate series
    ...
    if cf is not None:
        ...
        save_path = os.path.join(script_path, 'tsai_inceptiontime_cf.png')
