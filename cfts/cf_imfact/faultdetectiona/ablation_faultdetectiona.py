"""Ablation study for IMFACT counterfactual methods on FaultDetectionA.
...
def _save_sample_line_plot(
    ...
    true_class = _to_class_index(label)
    fig.suptitle(
        f"IMFACT Counterfactual Line Plots - FaultDetectionA | sample={sample_idx} | original={original_class} "
...
def main():
    parser = argparse.ArgumentParser(description="IMFACT ablation on FaultDetectionA.")
    ...
    print("=" * 80)
    print("IMFACT ABLATION - FaultDetectionA")
    print("=" * 80)
    ...
    output_prefix = args.output_prefix or os.path.join(SCRIPT_DIR, "faultdetectiona_ablations")
    ...
    for suite_name, configs in suite_configs.items():
        ...
        _print_summary(f"IMFACT {suite_name.upper()} SUMMARY - FaultDetectionA", methods, summary_rows)
        ...
        _save_summary_plot(plot_path, summary_rows, f"IMFACT {suite_name.replace('_', ' ').title()} on FaultDetectionA")
