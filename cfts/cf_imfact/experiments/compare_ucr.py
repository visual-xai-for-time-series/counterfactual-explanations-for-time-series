...
metric summary CSVs, bar charts, a UMAP projection, and line plots.
...
def _run_glacier(sample, dataset, model, target_class):
...
def _run_mascots(sample, dataset, model, target_class):
...
RUNNERS = {
    "imfact_default": _run_imfact,
    "native_guide": _run_native_guide,
    "wachter": _run_wachter,
    "glacier": _run_glacier,
    "mascots": _run_mascots,
}
    ...
    for i, idx in enumerate(selected_indices):
        ...
        for method_name in method_order:
            base = {
                ...
                "pred_cf": None,
                "l2_norm": np.nan, "normalized_distance": np.nan,
            ...
            _m = evaluate_counterfactual(
                s_cf, c_cf,
                model=model_fn, target_class=int(target_class),
...
def plot_waveforms(
    ...
    for sample_idx in selected_indices:
        if sample_idx not in all_cf_outputs:
            continue

        rep_payload = all_cf_outputs[sample_idx]
        x = _to_channel_first(rep_payload["sample"])[0]

        sample_results = results_df[results_df["sample_idx"] == sample_idx]
        ...
        out_path = os.path.join(out_dir, f"{dataset_name}_{sample_idx}.png")
        plt.savefig(out_path, dpi=120, bbox_inches="tight")
        plt.close()
...
def parse_args():
    ...
    parser.add_argument("--exclude-glacier", action="store_true",
                        help="Exclude GLACIER from the comparison")
    parser.add_argument("--exclude-mascots", action="store_true",
                        help="Exclude MASCOTS from the comparison")
...
def main():
    ...
    method_order = ["imfact_default", "native_guide"]
    if not args.exclude_wachter:
        method_order.append("wachter")
    if not args.exclude_glacier:
        method_order.append("glacier")
    if not args.exclude_mascots:
        method_order.append("mascots")
    ...
    if len(search_dataset) < len(dataset_test):
        print(f"Search dataset capped at {len(search_dataset)} samples for native_guide/wachter/glacier/mascots")
    ...
    plot_waveforms(
        ...
        dataset_name, method_order, os.path.join(out_dir, "lineplots"),
    )
