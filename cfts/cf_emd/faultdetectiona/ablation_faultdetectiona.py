"""Ablation study for EMD counterfactual methods on FaultDetectionA.

This script runs two separate ablations on a small set of correctly classified
FaultDetectionA test samples:
...
lightweight enough to run from the `cfts/cf_emd` package directory.
"""
...
from cfts.cf_emd.emd import _psd, emd_cf
...
def _save_sample_line_plot(
    ...
    true_class = _to_class_index(label)
    fig.suptitle(
        f"EMD Counterfactual Line Plots - FaultDetectionA | sample={sample_idx} | original={original_class} "
        f"({CLASS_NAMES.get(original_class, 'unknown')}) | target={target_class} ({CLASS_NAMES.get(target_class, 'unknown')})",
        fontsize=13,
        fontweight="bold",
    )
    ...
    axes[0].set_title(
        f"Original query sample - true: {true_class} ({CLASS_NAMES.get(true_class, 'unknown')})",
        fontsize=10,
    )
    ...
    for ax, outcome in zip(axes[1:], outcomes):
        ...
        if outcome.get("success"):
            ...
            ax.set_title(
                f"{method} [success] - pred: {outcome['pred_class']} ({CLASS_NAMES.get(int(outcome['pred_class']), 'unknown')})",
                fontsize=10,
            )
...
def _save_sample_umap_plot(
    ...
    for class_id in sorted(np.unique(class_ids)):
        ...
        ax.scatter(
            ...
            alpha=0.35,
            label=f"dataset {int(class_id)} ({CLASS_NAMES.get(int(class_id), 'unknown')})",
        )
...
def main():
    parser = argparse.ArgumentParser(description="EMD ablation on FaultDetectionA.")
    ...
    print("=" * 80)
    print("EMD ABLATION - FaultDetectionA")
    print("=" * 80)
    ...
    if args.model_file is None:
        model_file = os.path.join(REPO_ROOT, "models", f"simple_cnn_faultdetectiona_{output_classes}_len{input_length}.pth")
    else:
    ...
    output_prefix = args.output_prefix or os.path.join(SCRIPT_DIR, "emd_faultdetectiona_ablation")
    suite_configs = {
    ...
    for sample_rank, sample_idx in enumerate(selected_indices):
        ...
        print(f"\nSample {sample_idx}: original={original_class} ({CLASS_NAMES.get(original_class, 'unknown')}), target={target_class} ({CLASS_NAMES.get(target_class, 'unknown')})")
    ...
    for suite_name, configs in suite_configs.items():
        ...
        _print_summary(f"EMD {suite_name.upper()} SUMMARY - FaultDetectionA", methods, summary_rows)
        ...
        _save_summary_plot(plot_path, summary_rows, f"EMD {suite_name.replace('_', ' ').title()} on FaultDetectionA")
