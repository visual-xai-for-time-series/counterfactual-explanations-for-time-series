"""Ablation study for EMD counterfactual methods on FruitFlies.
...
def _save_sample_line_plot(
    ...
    true_class = _to_class_index(label)
    fig.suptitle(
        f"EMD Counterfactual Line Plots - {DATASET_NAME} | sample={sample_idx} | original={original_class} "
...
def main():
    parser = argparse.ArgumentParser(description=f"EMD ablation on {DATASET_NAME}.")
    ...
    print("=" * 80)
    print(f"EMD ABLATION - {DATASET_NAME}")
    print("=" * 80)
    ...
    output_prefix = args.output_prefix or os.path.join(SCRIPT_DIR, "emd_fruitflies_ablation")
    suite_configs = {
    ...
    for suite_name, configs in suite_configs.items():
        ...
        _print_summary(f"EMD {suite_name.upper()} SUMMARY - {DATASET_NAME}", methods, summary_rows)
        ...
        _save_summary_plot(plot_path, summary_rows, f"EMD {suite_name.replace('_', ' ').title()} on {DATASET_NAME}")
